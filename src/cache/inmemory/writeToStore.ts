...
function getChildMergeTree(
  ...
  let childTree = tree.map.get(name)!
  if (!childTree) tree.map.set(name, childTree = { map: new Map });
  return childTree;
}

function maybeRecycleChildMergeTree(tree: MergeTree, name: string | number) {
  const childTree = tree.map.get(name);
  if (childTree &&
      !childTree.info &&
      !childTree.map.size) {
    emptyMergeTreePool.push(childTree);
    tree.map.delete(name);
  }
}
